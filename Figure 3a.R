...
a <- seq(0.01, 1/2, length.out = 100)

#This function computes the correlation of the components of the asymptotic vector.
zcoor <-function(s,a){
  ((((1-(4*a*log(s)))*a*(4*a+2)*s^(-1))/(4*(pi^2)))  
     - ((18*a *s^3*log(s))/(pi^4)))/(sqrt(((((1-(4*a*log(s)))^2)/(4*(pi^2))) + ((18*(s^4)*(log(s))^2)/(pi^4)))*((((a^2)*((4*a+2)^2)*(s^(-2)))/(4*(pi^2)))+ ((18*(a^2)*(s^2))/(pi^4)))))  
}
...
######## Figure 3(a) - This figure gives the plot of the asymptotic correlation of hat{s_{0}} and hat{\alpha} based on the Shannon father wavelet.
persp3d(s,a,rho,col=pal[col.ind], box = FALSE,  xlab=" ", ylab=" ",zlab=" ")
...
#This gives Figure 3(a) in the PNG format.
rgl.snapshot("Fig3a.png")
